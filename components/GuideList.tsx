...
import { Tag, ArrowRight, Clock, BarChart, CheckCircle2, ListChecks, Search, Eye, CalendarDays, Sparkles, Loader2 } from 'lucide-react';
...
const GuideList: React.FC<GuideListProps> = ({ guides, searchTerm, onSearchChange, userId }) => {
  const { currentHero, settings, themeColor } = useApp();
  ...
  const categories = useMemo(() => {
    ...
    return [
        { id: 'Tümü', label: 'Tümü', count: guides.length },
  ...
  const filteredGuides = guides.filter((guide) => {
    ...
    const matchesCategory = selectedCategory === 'Tümü' || guide.category === selectedCategory;
    ...
    <div className="space-y-8 animate-fade-in">
                    ...
                    <>
                        <div className="flex justify-between items-center mb-2">
                             <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                 <Sparkles size={12} className="text-indigo-500" /> Yeni Konu:
                             </span>
      ...
      {filteredGuides.length > 0 ? (
        <>
            ...
            {filteredGuides.slice(0, firstBatchSize).map((guide) => <GuideCard key={guide.id} guide={guide} progressMap={progressMap} />)}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGuides.slice(firstBatchSize).map((guide) => <GuideCard key={guide.id} guide={guide} progressMap={progressMap} />)}
            </div>
        </>
      ) : (
        <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
          ...
          <button onClick={() => {onSearchChange(''); setSelectedCategory('Tümü');}} className="mt-2 text-brand-600 dark:text-brand-400 text-sm font-bold hover:underline">Filtreleri Temizle</button>
        </div>
      )}
    </div>
  );
...
const GuideCard = ({ guide, progressMap }: any) => {
    ...
    return (
        <Link to={`/guide/${guide.slug || guide.id}`} className={`group bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-brand-400 dark:hover:border-brand-500 hover:shadow-lg transition-all duration-300 flex flex-col h-full overflow-hidden ${isCompleted ? 'opacity-75' : ''}`}>
            <div className="h-40 overflow-hidden relative bg-slate-100 dark:bg-slate-900">
                ...
                {isCompleted && <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-[1px]"><span className="bg-green-500 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle2 size={12} /> Tamamlandı</span></div>}
            </div>
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                {(hasProgress || isCompleted) && (
                    <div className="mb-3">
                         <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">
                            <span className="flex items-center gap-1"><ListChecks size={12} /> {completedCount}/{totalSteps} Adım</span>
                ...
                <div className="mt-auto pt-3 border-t border-slate-50 dark:border-slate-700 flex items-center justify-between text-xs font-bold text-brand-600 dark:text-brand-400">
                    <span>{isCompleted ? 'Tekrar Oku' : (hasProgress ? 'Devam Et' : 'Başla')}</span>
