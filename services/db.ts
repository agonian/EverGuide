...
import { collection, getDocs, doc, setDoc, deleteDoc, getDoc, query, where, updateDoc, disableNetwork, onSnapshot } from "firebase/firestore";
...
export const DataService = {
    ...
    // 1.1 Subscribe to Guides (Real-time)
    subscribeToGuides: (callback: (guides: Guide[]) => void): () => void => {
        if (!isFirebaseEnabled || !db || isQuotaExceeded) {
            callback([]);
            return () => {};
        }
        ...
        try {
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                const dbGuides = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Guide));
                const sortedGuides = dbGuides.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
                callback(sortedGuides);
            }, (error) => {
                console.error("Error in guides snapshot:", error);
                if (error.code === 'resource-exhausted') setQuotaExceeded();
            });
            return unsubscribe;
        } catch (e) {
            console.error("Error setting up guides subscription:", e);
            return () => {};
        }
    },
    ...
    // 7.1 Subscribe to Settings (Real-time)
    subscribeToSettings: (callback: (settings: SiteSettings) => void): () => void => {
        if (!isFirebaseEnabled || !db || isQuotaExceeded) {
        ...
        try {
            ...
            console.error("Error setting up settings subscription:", e);
            return () => {};
        }
    },
                ...
                config: { 
                    ...
                    responseSchema: { 
                        ...
                        properties: { 
                            ...
                            imagePrompt: { type: Type.STRING, description: "Konuyla ilgili, blog yazısı kapağı olabilecek, modern, temiz, yüksek kaliteli, fotogerçekçi bir görsel için İngilizce detaylı prompt." },
                            steps: { 
                                type: Type.ARRAY, 
                                items: { 
                                    type: Type.OBJECT, 
                        ...
                        required: ["title", "category", "difficulty", "duration", "description", "imageKeyword", "imagePrompt", "steps"] 
                    } 
                } 
            });
    ...
    checkAndTriggerAutoGenerate: async (): Promise<boolean> => {
        ...
        try {
            ...
            if (generatedData && generatedData.title) {
                ...
                let imageUrl = generatedData.imageUrl || `https://picsum.photos/seed/${slug}/800/600`;

                // If image is Base64, upload it to server to avoid Firestore 1MB limit
                if (imageUrl.startsWith('data:image')) {
                    try {
                        const uploadResponse = await fetch('/api/upload-image', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ image: imageUrl, slug })
                        });
                        if (uploadResponse.ok) {
                            const uploadData = await uploadResponse.json();
                            imageUrl = uploadData.url;
                        } else {
                            console.warn("Auto-gen image upload failed, falling back to placeholder");
                            imageUrl = `https://picsum.photos/seed/${slug}/800/600`;
                        }
                    } catch (uploadErr) {
                        console.error("Auto-gen image upload error:", uploadErr);
                        imageUrl = `https://picsum.photos/seed/${slug}/800/600`;
                    }
                }
